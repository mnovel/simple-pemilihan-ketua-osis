...
function cekLogin($data, $username)
{
    ...
    if ($fetch[4] == 0 | $ip == $fetch[5]) {
        $date = time();
        mysqli_query($connection, "UPDATE moderator SET ip='${ip}',login='1' WHERE username='$username'");
...
{
    global $connection;
    $login = mysqli_query($connection, "SELECT * FROM moderator WHERE username='$username' AND password='$password'");
