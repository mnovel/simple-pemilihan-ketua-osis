...
if (empty($_GET['id'])) {
    setcookie('alert', "toastr.error('Please select the admin first')", time() + 5, '/');
    ...
    curl_setopt($ch, CURLOPT_URL, $url . "?table=$table&id=$id&data=admin&type=id&api_key=$apikey");
...
if (!empty($_POST['name'])) {
    ...
    $cek = mysqli_query($connection, "UPDATE admin SET name='$name', username='$username', password='$password', login='$login' WHERE id='$id'");

    if ($cek) {
        setcookie('alert', "toastr.success('Success update admin')", time() + 5, '/');
        header('location:admin');
    } else {
        setcookie('alert', "toastr.error('Failed update admin. ')" . mysqli_error($connection), time() + 5, '/');
        header('location:admin');
    }
}
